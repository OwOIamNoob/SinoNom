...
def warp_transform(img, 
  ...
  # print("Matching size:{0},{1}".format(w_new, h_new))
  masked_img = np.concatenate([img, mask[:, :, None].astype(np.uint8)], axis=2)
...
def augment_img(img, rotate=0, shear_x=0, shear_y=0, noise=0, logger = None, debug = None, keep_mask = False, export= False, borderMode='constant'):
  # cài đặt thông số
  skew_angle = 0
  shear_x_level = 0
  shear_y_level = 0
